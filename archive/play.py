...
Key design point: helper functions have DIFFERENT signatures.
  - Some take only (env, agent_id)          -> e.g. random_explore
...
MODEL_NAME = "Qwen/Qwen3-4B-Instruct-2507" # Using a capable small model for demo purposes
...
ACTION_REGISTRY = {
    ...
    "clean_at": {
        "func": helpers.clean_at,
        "has_args": True,
        "arg_keys": ["coord_x", "coord_y"],
    },
    "random_explore": {
        "func": helpers.random_explore,
        "has_args": False,
        "arg_keys": [],
    },
}
...
def parse_action(text: str) -> dict:
    ...
    nested_match = re.search(r'\{.*"args"\s*:\s*\{[^{}]*\}.*\}', text, re.DOTALL)
    if nested_match:
        try:
...
def execute_action(env, agent_id: int, parsed: dict) -> str:
    """Call the helper function from ACTION_REGISTRY and return the low-level action string.
    ...
    Returns:
        str: The low-level action string to pass to env.step().
    """
    ...
    if entry["has_args"]:
        ...
        if missing:
            print(f"  [WARN] Action '{action_name}' missing args {missing}; defaulting to random_explore")
            # Fallback to random explore if args are missing
            action_str, _ = helpers.random_explore(env, agent_id)
            return action_str

        # Call with unpacked kwargs: func(env, agent_id, coord_x=5, coord_y=3, ...)
        # Ensure args are integers if they look like numbers
        clean_args = {}
        ...
        action_str, _is_done = func(env, agent_id, **clean_args)
    else:
        # Call with just (env, agent_id)
        action_str, _is_done = func(env, agent_id)
...
def decide_action(tokenizer, model, env, agent_id: int) -> tuple:
    ...
    action_msgs = prompt_template.create_single_stage_prompt(env, thinking_response, agent_id)
...
def run_episode(tokenizer, model):
    ...
    cfg = Config(
        ...
        seed=42,
        # Ensure we have some dirt and apples to start
        init_dirt_prob=0.3,
        apple_spawn_base=0.1
    )
    ...
    for step in range(1, MAX_ENV_STEPS + 1):
        ...
        for agent_id in range(1, NUM_AGENTS + 1):
            ...
            action_str = execute_action(env, agent_id, parsed)
            low_level_actions[agent_id] = action_str
            
            print(f"--- Step {step} | Agent {agent_id} ---")
            print(f"  Observation: {helpers.get_observation_description(env, agent_id)}")
            print(f"  Thinking: {thinking}")
            print(f"  Raw action output: {raw_action}")
            print(f"  Parsed: {parsed}")
            print(f"  Low-level Action: {action_str}")
    ...
    print("=" * 60)
    print(f"  Steps completed : {final_info.get('step', 0)}")
    print(f"  Final scores    : {final_info.get('scores', {})}")
    print(f"  Total rewards   : {dict(total_rewards)}")
    print(f"  Final dirt count: {final_info.get('dirt_count', 0)}")
    print(f"  Final apple cnt : {final_info.get('apple_count', 0)}")
    if step_times:
