"""
train.py — GRPO training for CleanupEnvMove multi-agent game.

Generated from grpo_template.py. All # === CUSTOMIZE === sections have been
filled in with CleanupEnvMove-specific implementations.

Usage:
    accelerate launch train.py [options]
    python train.py --num_episodes 10   # debug run

Logging:
    train.logs       — Training statistics (loss, reward, lr, gradNorm, etc.)
    Sample_EP.logs   — Sample episode trajectories (logged every 10 episodes)
"""
...
def setup_logging(output_dir: str, is_main: bool):
    """Configure logging to both console and train.logs file."""
    ...
    if is_main:
        train_log_path = os.path.join(output_dir, "train.logs")
        file_handler = logging.FileHandler(train_log_path, mode="a")
...
# ============================================================
# SAMPLE EPISODE LOGGER
# ============================================================

class SampleEpisodeLogger:
    """
    Logs full sample episodes to Sample_EP.logs every N episodes.
    Captures: prompts, thinking outputs, action outputs, environment state transitions.
    """

    def __init__(self, output_dir: str, log_every: int = 10):
        self.log_path = os.path.join(output_dir, "Sample_EP.logs")
        self.log_every = log_every
        self.episode_count = 0

    def should_log(self, episode_num: int) -> bool:
        return episode_num % self.log_every == 0
    ...
    def log_episode(
        ...
        """
        Append one full episode to Sample_EP.logs.

        Args:
            episode_num  : Global episode number
            group_num    : Training group number
            trajectory   : Episode trajectory dict
            env_states   : List of env.get_state() snapshots per step (after step)
            num_agents   : Number of agents
        """
        with open(self.log_path, "a", encoding="utf-8") as f:
            f.write("\n" + "=" * 100 + "\n")
            f.write(f"SAMPLE EPISODE | Group {group_num} | Episode {episode_num}\n")
            f.write(f"Total Reward: {trajectory.get('total_reward', 0):.2f} | "
                    f"Steps: {trajectory.get('steps', 0)} | "
                    f"Final Scores: {trajectory.get('final_scores', {})}\n")
            f.write("=" * 100 + "\n\n")

            steps = trajectory.get("steps", 0)
            for step_idx in range(steps):
                f.write(f"{'─'*80}\n")
                f.write(f"STEP {step_idx + 1}/{steps}\n")
                f.write(f"{'─'*80}\n")

                # Environment state after this step
                if step_idx < len(env_states):
                    state = env_states[step_idx]
                    f.write(f"[ENV STATE] Scores: {state.get('scores', {})} | "
                            f"Step: {state.get('step_count', '?')}\n")
                    # Render grid-like info
                    agents_pos = state.get("agents", {})
                    f.write(f"[ENV STATE] Agent positions: {agents_pos}\n")

                # Per-agent info
                for a_idx in range(num_agents):
                    global_idx = step_idx * num_agents + a_idx
                    if global_idx >= len(trajectory.get("agent_ids", [])):
                        break

                    agent_id = trajectory["agent_ids"][global_idx]
                    obs_text = trajectory["observations"][global_idx] if global_idx < len(trajectory.get("observations", [])) else "N/A"
                    action = trajectory["actions"][global_idx] if global_idx < len(trajectory.get("actions", [])) else "N/A"
                    reward = trajectory["rewards"][global_idx] if global_idx < len(trajectory.get("rewards", [])) else 0.0
                    thinking = trajectory["responses"][global_idx] if global_idx < len(trajectory.get("responses", [])) else "N/A"
                    action_text = trajectory["action_texts"][global_idx] if global_idx < len(trajectory.get("action_texts", [])) else "N/A"

                    # Truncate long texts for readability
                    thinking_display = thinking[:500] + "..." if len(thinking) > 500 else thinking
                    obs_display = str(obs_text)[:300] + "..." if len(str(obs_text)) > 300 else str(obs_text)

                    f.write(f"\n  [Agent {agent_id}]\n")
                    f.write(f"    OBS    : {obs_display}\n")
                    f.write(f"    THINK  : {thinking_display}\n")
                    f.write(f"    ACTION : {action}\n")
                    f.write(f"    OUTPUT : {action_text[:200]}\n")
...
class AllowOnlyActionWords(LogitsProcessor):
    ...
    Used during Stage 2 to guarantee valid JSON action output.
    """
...
class CleanupGRPOTrainer:
    """
    GRPO trainer for CleanupEnvMove multi-agent game.

    Two-stage generation:
    - Stage 1: Model reasons about the situation (thinking)
    - Stage 2: Model outputs a JSON action given the reasoning

    Actions are dispatched to helper functions that call env.step().
    """
    ...
    def __init__(self, config: GRPOConfig):
        ...
        # Setup logging early so we can log to file
        setup_logging(config.output_dir, is_main)
        ...
        # ------------------------------------------------------------------
        # === CUSTOMIZE ===
        # Action vocabulary: the agent outputs a JSON action name string.
        # These are the action names (keys) in the action registry.
        # ------------------------------------------------------------------
        self.action_words: List[str] = list(ACTION_REGISTRY.keys())
        # action_words = ["move_to", "eat", "clean", "stay", "move_up",
        ...
        # ------------------------------------------------------------------
        # === CUSTOMIZE ===
        # Environment configuration for CleanupEnvMove.
        # ------------------------------------------------------------------
        self.env_config = EnvConfig(
            ...
            seed=config.seed,
            dirt_spawn_prob=0.02,
            apple_spawn_base=0.05,
            apple_cleanliness_exponent=1.0,
            max_apples=5,
            max_dirts=None,
            init_dirt_prob=0.35,
        )
        ...
        # Sample episode logger
        self.sample_logger = SampleEpisodeLogger(
    ...
    def get_action_from_response(self, response: str, agent_id: int = 1) -> str:
        """
        Extract a valid action name from model output.

        The model is expected to output a JSON like:
          {"action": "move_to", "agent_id": 1, "args": {"coord_x": 5, "coord_y": 3}}

        We parse this and return the "action" field value.
        Falls back to "stay" on parse failure.

        Args:
            response : Raw decoded text from the model.
            agent_id : The agent making the action (for validation/fixing).

        Returns:
            A string action name in self.action_words.
        """
        parsed = parse_action_json(response)
        if parsed is not None:
            action_name = parsed.get("action", "stay")
            if action_name in self.action_words:
                return action_name
            # Fuzzy match
            for aw in self.action_words:
                if aw in action_name.lower():
                    return aw

        # Fallback: scan text for action keywords
        response_lower = response.strip().lower()
        for aw in self.action_words:
            if aw in response_lower:
                return aw

        logger.debug(f"No valid action in response '{response[:100]}', defaulting to 'stay'")
        return "stay"

    def parse_full_action(self, response: str, agent_id: int) -> dict:
        """
        Parse the full action JSON from model output.

        Returns a dict with 'action', 'agent_id', 'args' keys,
        or a default stay action on failure.
        """
        parsed = parse_action_json(response)
        if parsed is None:
            return {"action": "stay", "agent_id": agent_id, "args": {}}
    ...
    def _generate_eval_states(self, num_states: int = 20):
        """Pre-generate fixed initial states for reproducible evaluation."""
    ...
    def compute_batch_sequence_log_prob(
        ...
        """Compute summed log-probability of generated tokens."""
    ...
    def generate_action(
        ...
        # ---- Stage 2: action ----------------------------------------------
        action_prompt = self.create_action_prompt_for_agent(obs_text, thinking_text, agent_id)
        ...
        action = self.get_action_from_response(action_text, agent_id)
    ...
    def generate_actions_batch(
        ...
        print("DEBUG: Thinking texts:", thinking_texts)
        # ---- Stage 2 batch ------------------------------------------------
        action_prompts = [
            self.create_action_prompt_for_agent(obs_texts[i], thinking_texts[i], agent_ids[i])
            for i in range(n)
        ]
        a_inputs = self.tokenizer(
        ...
        for i in range(n):
            ...
            actions.append(self.get_action_from_response(a_text, agent_ids[i]))
    ...
    def run_episode(
        ...
        initial_state=None,
        capture_env_states: bool = False,
    ) -> Dict:
        """
        Run one full episode and collect the trajectory.

        Returns a dict with trajectory data + optionally env_states for logging.
        """
        ...
        env = CleanupEnvMove(EnvConfig(
            ...
            river_width=3,
            max_steps=self.config.max_env_steps,
            seed=self.config.seed + random.randint(0, 100000),
        ))
        ...
        env_states = [] if capture_env_states else None
        total_reward = 0.0
        ...
        for step in range(self.config.max_env_steps):
            ...
            for agent_id in range(1, self.config.num_agents + 1):
                ...
                trajectory["observations"].append(obs_text)
                ...
                if log_samples and step == 0 and agent_id == 1:
                    logger.info(
                        f"  Sample: obs='{obs_text[:80]}' "
                        f"thinking='{thinking_text[:80]}' action='{action}'"
                    )
        ...
        if capture_env_states:
            trajectory["env_states"] = env_states
    ...
    def flatten_trajectories(self, trajectories: List[Dict]) -> List[Dict]:
        """Convert trajectory dicts into flat list of step-level samples."""
        samples = []
    ...
    def train(self):
        ...
        if is_main:
            ...
            logger.info("=" * 70)
            logger.info("GRPO Training — CleanupEnvMove")
        ...
        while episode < self.config.num_episodes:
            ...
            # Decide if we should capture env states for sample logging
            should_log_sample = (
                is_main
                and self.sample_logger.should_log(episode)
                and self.config.log_trajectory
            )
            ...
            if self.accelerator is not None:
                ...
                for i in range(self.config.episodes_per_gpu):
                    try:
                        ...
                        capture = should_log_sample and i == 0 and self.accelerator.is_main_process
                        traj = self.run_episode(log_samples=log_samples, capture_env_states=capture)
                ...
                for i in range(self.config.episodes_per_update):
                    try:
                        ...
                        capture = should_log_sample and i == 0
                        traj = self.run_episode(log_samples=log_samples, capture_env_states=capture)
                        trajectories.append(traj)
                        logger.info(
            ...
            # Log sample episode to Sample_EP.logs
            if is_main and should_log_sample and self.config.log_trajectory:
                sample_traj = trajectories[0]
                env_states = sample_traj.pop("env_states", [])
                self.sample_logger.log_episode(
                    episode_num=episode,
                    group_num=group_num,
            ...
            # Log one episode for monitoring (standard trajectory log)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="GRPO Training — CleanupEnvMove")
    parser.add_argument("--model_name",         default=None)
    parser.add_argument("--num_episodes",        type=int,   default=None)
    parser.add_argument("--num_agents",          type=int,   default=None)
    parser.add_argument("--episodes_per_update", type=int,   default=None)
    parser.add_argument("--episodes_per_gpu",    type=int,   default=None)
    parser.add_argument("--learning_rate",       type=float, default=None)
    parser.add_argument("--output_dir",          default=None)
    parser.add_argument("--use_wandb",           action="store_true")
    parser.add_argument("--seed",                type=int,   default=None)
    parser.add_argument("--max_env_steps",       type=int,   default=None)
    parser.add_argument("--thinking_tokens",     type=int,   default=None)
    ...
    # Set model to Qwen3-4B as specified
    if config.model_name == "Qwen/Qwen2.5-1.5B":
        config.model_name = "Qwen/Qwen3-4B-Instruct-2507"
