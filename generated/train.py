"""
train.py — GRPO training for LLM agents in CleanupEnvMove.

Generated from grpo_template.py with CUSTOMIZE sections filled in.
"""
...
# === CUSTOMIZE ===
# Import environment, helpers, and prompt templates
...
from helpers import observation_to_text
from prompt_template import create_thinking_prompt, create_single_stage_prompt, ACTION_WORDS
# ==================
...
class CleanupGRPOTrainer:
    """
    GRPO trainer for CleanupEnvMove.
    """
    ...
    def __init__(self, config: GRPOConfig):
        ...
        # ------------------------------------------------------------------
        # === CUSTOMIZE ===
        # Action vocabulary: simple directional actions matching env_move.py
        # ------------------------------------------------------------------
        self.action_words: List[str] = ACTION_WORDS
    ...
    def format_observation(self, raw_obs, agent_id: int, env) -> str:
        ...
        Uses observation_to_text from helpers.py.
        """
    ...
    def create_thinking_prompt(self, obs_text: str, agent_id: int) -> str:
        ...
        Uses create_thinking_prompt from prompt_template.py.
        """
    ...
    def create_action_prompt(self, obs_text: str, thinking_text: str) -> str:
        ...
        Uses create_single_stage_prompt from prompt_template.py.
        """
        # agent_id is not critical for stage 2; use 0 as placeholder
        messages = create_single_stage_prompt(obs_text, thinking_text, agent_id=0)
        return self.tokenizer.apply_chat_template(
            messages, tokenize=False, add_generation_prompt=True
        )
    ...
    def get_action_from_response(self, response: str) -> str:
        ...
        Scans the response for known action words.
        """
        response_lower = response.strip().lower()

        # Try to parse JSON first (in case model outputs {"action": "up"})
        try:
            data = json.loads(response_lower)
            if isinstance(data, dict) and "action" in data:
                action = data["action"].lower().strip()
                if action in self.action_words:
                    return action
        except (json.JSONDecodeError, AttributeError):
            pass

        # Scan words from the end for a known action
    ...
    def compute_batch_sequence_log_prob(
        ...
        """Compute summed log-probability of generated tokens per sequence."""
    ...
    def run_episode(
        ...
        # === CUSTOMIZE ===
        # Create and reset the CleanupEnvMove environment
        env = CleanupEnvMove(EnvConfig(
            n_agents=self.config.num_agents,
            max_steps=self.config.max_env_steps,
        ...
        for step in range(self.config.max_env_steps):
            ...
            # === CUSTOMIZE ===
            # Step the environment with all agents' actions
    ...
    def flatten_trajectories(self, trajectories: List[Dict]) -> List[Dict]:
        """Convert trajectory dicts into a flat list of step-level samples."""
        samples = []
    ...
    def compute_loss_on_samples(
        ...
        """Compute PPO-clipped GRPO loss on a micro-batch."""
        if not samples:
